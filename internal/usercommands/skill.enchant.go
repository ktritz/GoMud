...
func Enchant(rest string, user *users.UserRecord, room *rooms.Room, flags events.EventFlag) (bool, error) {
	...
	if !found {
		...
		*/
		...
		chanceToDestroy -= user.Character.Stats.ActionValueAdj("EnchantProtection") >> 2 // 1% less chance for each 4 mysticism points
		...
		if matchItem.GetSpec().Type == items.Weapon {
			damageBonus = int(math.Ceil(math.Sqrt(float64(user.Character.Stats.ActionValueAdj("EnchantPower")))))
		}

		// At skill level 2, can enchant weapons and armor
		if skillLevel >= 2 && matchItem.GetSpec().Subtype == items.Wearable {
			defenseBonus = int(math.Ceil(math.Sqrt(float64(user.Character.Stats.ActionValueAdj("EnchantPower")))))
		}
		...
		if skillLevel >= 3 {
			...
			for i := 0; i < bonusCt; i++ {
				...
				statBonus[chosenStat] = int(math.Ceil(math.Sqrt(float64(user.Character.Stats.ActionValueAdj("EnchantPower")))))
			}
		}
